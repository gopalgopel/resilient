...
var dishidros = L.esri.dynamicMapLayer({url: 'http://hdc.pushidrosal.id/arcgis/rest/services/SampleWorldCities/MapServer/exts/MaritimeChartService/MapServer', opacity: 0.6, f:'image', layers: [0,2,3,4,5,6,7,8,9,10]});
//layer.show:
// 0 = kotak batas
...
var buletdefault = L.icon({
  iconUrl: 'aset/img/icondefault.png',      
  iconSize:     [12, 12], 
  iconAnchor:   [6, 6], 
});
var bulethijau = L.icon({
  iconUrl: 'aset/img/iconhijau.png',      
  iconSize:     [12, 12], 
  iconAnchor:   [6, 6], 
});
var bulethitam = L.icon({
  iconUrl: 'aset/img/iconhitam.png',      
  iconSize:     [12, 12], 
  iconAnchor:   [6, 6], 
});
var buletkuning = L.icon({
  iconUrl: 'aset/img/iconkuning.png',      
  iconSize:     [12, 12], 
  iconAnchor:   [6, 6], 
});
var buletmerah = L.icon({
  iconUrl: 'aset/img/iconmerah.png',      
  iconSize:     [12, 12], 
  iconAnchor:   [6, 6], 
});
var buletputih = L.icon({
  iconUrl: 'aset/img/iconputih.png',      
  iconSize:     [12, 12], 
  iconAnchor:   [6, 6], 
});
var bulettransp = L.icon({
  iconUrl: 'aset/img/icontransp.png',      
  iconSize:     [12, 12], 
  iconAnchor:   [6, 6], 
});
var buletungu = L.icon({
...
//INIT RESILIENT//INIT RESILIENT//INIT RESILIENT
//INIT RESILIENT//INIT RESILIENT//INIT RESILIENT

var lapor3= L.layerGroup([]),lapor7= L.layerGroup([]),lapor30= L.layerGroup([]),lapor365= L.layerGroup([]);
$.ajax({
  type: 'GET',        
  url: 'http://192.168.1.241:9099/api/rawpesans/filter/last3day',        
...
$.ajax({
  type: 'GET',        
  url: 'http://192.168.1.241:9099/api/rawpesans/filter/lastweek',        
  success:function(lapor){
    console.log(lapor);
    for(var i=0; i<lapor.data.length; i++){
      var mark = L.marker([lapor.data[i].lokasi.latitude, lapor.data[i].lokasi.longitude], {icon:buletmerah})
...
$.ajax({
  type: 'GET',        
  url: 'http://192.168.1.241:9099/api/rawpesans/filter/lastmonth',        
  success:function(lapor){
    console.log(lapor);
    for(var i=0; i<lapor.data.length; i++){
      var mark = L.marker([lapor.data[i].lokasi.latitude, lapor.data[i].lokasi.longitude], {icon:buletmerah})
...
$.ajax({
  type: 'GET',        
  url: 'http://192.168.1.241:9099/api/rawpesans/filter/lastyear',        
  success:function(lapor){
    console.log(lapor);
    for(var i=0; i<lapor.data.length; i++){
      var mark = L.marker([lapor.data[i].lokasi.latitude, lapor.data[i].lokasi.longitude], {icon:buletmerah})
...
var resilient = {
  ...
  "Personel":{
    "<font color='#FFC100'> AS <i class='fa fa-star'></i></font> ": KRI,
    "<font color='#FBFF00'> AD <i class='fa fa-star-half-o'></i></font>" : LANAL,
    "<font color='#61FF00'> ME <i class='fa fa-star-o'></i></font>" : MARINIR,
    "<font color='red'> Out <i class='fa fa-frown-o'></i></font>" : SATGAS,
  }
};
...
var bas = "/getcuaca/bin/hasil/"; 
tekananUrl = bas+'psf_'+k+'.png';
suhuUrl = bas+'sst_'+k+'.png';
hujanUrl = bas+'rain_'+k+'.png';
awanUrl = bas+'cloud_'+k+'.png';
...
// var bmkg = bas+"gempaauto.xml";
  ...
  // awan = L.layerGroup(L.imageOverlay(awanUrl, smBounds));
  // tekanan = L.layerGroup(L.imageOverlay(tekananUrl, smBounds));
  // suhu = L.layerGroup(L.imageOverlay(suhuUrl, smBounds));
  // hujan = L.layerGroup(L.imageOverlay(hujanUrl, smBounds));
...
var urlbatasL = 'json/batasline.json';  
...
var layerControl = L.control.layers(bsm);
...
WindJSLeaflet.init({ 
    ...
    // pingUrl: 'http://192.168.1.124:7000/alive',        // url to check service availability
    latestUrl: 'http://192.168.1.124:7000/latest',     // url to get latest data with no required params   
    nearestUrl: 'http://192.168.1.124:7000/nearest',   // url to get data nearest a specified time ISO 
